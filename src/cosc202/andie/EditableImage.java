...
class EditableImage {
    ...
    /** The Stack of image operations for a macron*/
    private Stack<ImageOperation> macroStack;
    /**boolean filed to edit the state of the macro */
    protected static boolean macroRunning = false;
    /**counter for checking number of operations */
    ...
    /** 
     * <p>
     * Exports the edited image to the specifed file path.
     * </p>
     * <p>
     * This method applies all the operations stored in the <code>ops</code> list to 
     * the origional image and saves the resuting image as a JEPG file to the <code>outputFilePath</code>.
     * </p>
     * @param outputFilePath The file path where the edited image will be saved as a JPG. 
     * @throws Exception If an error occurs while applying operations or writing the edited image to the file.
     */
    
    public void export(String outputFilePath) throws Exception {
        ...
        // saving the edited image as a JPG file
        ImageIO.write(editedImage, "jpg", new File(outputFilePath));
    ...
    public void apply(ImageOperation op) {
        ...
        if (macroRunning) {
            macroStack.add(op);
            countOperations++; // increment the operationCount if macroRunning is true
        }
        ops.add(op);
    }
...
/**
 * Loads a macro that is a sequence of operations from a file.
 * The file contains a serialized Stack of ImageOperation objects.
 * The operations are loaded onto the macroStack in reverse order.
 *
 * @param filePath The path of the file from which to load the macro.
 * @throws Exception If an error occurs during file reading or deserialization, or if the file does not exist.
 */
...
/**
 * Saves the current sequence of operations to a file of type macro.
 * If the file doenst have .macro as it's suffix then it is added.
 * The file is then saved in serialized form using object serialization.
 *
 * @param fileName The name of the file to save the macro to. 
 * @throws Exception If an error occurs during file writing or serialization, or if the macro stack is empty.
 */
