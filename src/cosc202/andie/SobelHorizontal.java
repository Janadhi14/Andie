 ...
 * 
 * <p> The sobel horizontal filter applys a convolution operation to the image. </p>
 * 
 * @author Bernice
 */
...
public class SobelHorizontal implements ImageOperation, java.io.Serializable {
    /**
     * <p>
     * Apply a horinzontal sobel filter to an image.
     * </p>
     * 
     * <p>
     * 
    ...
    public BufferedImage apply(BufferedImage image) {
        ...
        for (int y = 1; y < height - 1; y++) {
            for (int x = 1; x < width - 1; x++) {
                int pixelX = ((image.getRGB(x + 1, y - 1) & 0xFF) - (image.getRGB(x - 1, y - 1) & 0xFF))
                        + 2 * ((image.getRGB(x + 1, y) & 0xFF) - (image.getRGB(x - 1, y) & 0xFF))
                        + ((image.getRGB(x + 1, y + 1) & 0xFF) - (image.getRGB(x - 1, y + 1) & 0xFF));

                // Normalize the pixel value
                int normalizedPixel = Math.abs(pixelX) > 255 ? 255 : Math.abs(pixelX);
